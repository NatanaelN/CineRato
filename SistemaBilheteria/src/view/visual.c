...
void exibirSala(Bilheteria *bilheteria, int sala) {
    ...
    for (int i = 0; i < MAX_LINHAS; i++) {
        ...
        for (int j = 0; j < MAX_COLUNAS; j++) {
            ...
            if (h4 == 'X') {
                printf(" %s%c%s ", VERM_BASICO, bilheteria->sala[sala][i][j],
                       LIMPAR);
            } else {
...
void imprimirBilhete(const Bilheteria *B, int idx) {
    ...
    char *itens_bilhete[] = {
        "Bilhete", "Filme", "Sala", "Poltrona", "Cliente", "Ingresso", NULL
    };
    ...
    snprintf(buffer_bilhete, sizeof(buffer_bilhete), "%d",
             B->bilhetes[idx].numero_bilhete);

    char buffer_cadeira[4];
    snprintf(buffer_cadeira, sizeof(buffer_cadeira), "%c%d",
             'A' + (B->bilhetes[idx].fileira - 1), B->bilhetes[idx].coluna);

    char *valor_bilhete[] = {
        buffer_bilhete, B->bilhetes[idx].nome_filme, buffer_sala,
        buffer_cadeira, B->bilhetes[idx].nome_cliente,
        B->bilhetes[idx].tipo_ingresso, NULL
    };
    const int maiorTamanhoValor = encontrarMaiorTamanhoString(valor_bilhete);
    int t_caixa2 = ((maiorTamanho + maiorTamanhoValor + 10) <= 58)
                       ? 58
                       : (maiorTamanho + maiorTamanhoValor + 10);
    ...
    for (int i = 0; itens_bilhete[i] != NULL; i++) {
        printf("\t\t\xBA %s%-*s%s | %-*s", AZUL_BASICO, maiorTamanho,
               itens_bilhete[i], LIMPAR, maiorTamanhoValor, valor_bilhete[i]);
...
void listarBilhetes(Bilheteria *B, int criterio) {
    ...
    switch (criterio) {
        case ORDEM_CRESCENTE:
            qsort(B->bilhetes, B->num_bilhetes, sizeof(Bilhete),
        ...
        case ORDEM_DECRESCENTE:
            qsort(B->bilhetes, B->num_bilhetes, sizeof(Bilhete),
                  compararBilhetesPorNumeroDecrescente);
            printf("\t\t");
            centralizar_nomepadrao("Ordenacao: Decrescente", 98, 1);
            break;
        case ORDEM_TIPO:
            qsort(B->bilhetes, B->num_bilhetes, sizeof(Bilhete),
                  compararBilhetesPorTipoIngresso);
            printf("\t\t");
