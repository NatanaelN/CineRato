    ...
    for (int i = 0; i < bilheteria->num_filmes; ++i) {
        for (int j = 0; j < bilheteria->num_filmes; ++j) {
            if (filmes[i].data_lancamento.mes > filmes[j].data_lancamento.mes) {
                ...
                filmes[i] = filmes[j];
                filmes[j] = temp;
            } else if (filmes[i].data_lancamento.dia > filmes[j].data_lancamento
                       .dia && filmes[i].data_lancamento.mes == filmes[j].
                       data_lancamento.mes) {
                Filme temp = filmes[i];
                filmes[i] = filmes[j];
                filmes[j] = temp;
            } else if (filmes[i].data_lancamento.ano > filmes[j].data_lancamento
                       .ano) {
                Filme temp = filmes[i];
                filmes[i] = filmes[j];
                filmes[j] = temp;
